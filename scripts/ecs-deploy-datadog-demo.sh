...
docker ps --format '{{.Names}}' | grep -E '^(demo-spring-app|fx-python|datadog-agent|otel-collector|frontend-proxy|frontend|ad|cart|checkout|currency|email|image-provider|load-generator|payment|product-catalog|quote|recommendation|shipping|flagd|flagd-ui|telemetry-docs|astronomy-db|valkey-cart)$' || true
if [[ -d "$APP_DIR" ]]; then
  cd "$APP_DIR"
...
docker exec datadog-agent agent status || true
